...
def main(args):
    ...
    channel_stats = {
        channel: {'total_images': 0, 'total_annotations': 0, 'total_predicted_boxes': 0, 'correct_predictions': 0}
    ...
    for image_path in images:
        ...
        if channel in channels:
            ...
            # Filter predicted boxes based on confidence threshold
            confidence_mask = outputs[0]['scores'].cpu().numpy() >= args['confidence']
            pred_boxes = outputs[0]['boxes'].cpu().numpy()[confidence_mask]

            # Take the box with the highest confidence if there are multiple boxes
            if len(pred_boxes) > 0:
                max_conf_box = pred_boxes[np.argmax(outputs[0]['scores'].cpu().numpy()[confidence_mask])]
                pred_boxes = np.array([max_conf_box])
            ...
            if os.path.exists(xml_path):
                ...
                if not isinstance(target_boxes, list):
                    target_boxes = [target_boxes]  # Convert scalar to list
                ...
                for pred_box in pred_boxes:
                    ...
                    if max_iou >= args['iou_threshold']:
                        channel_stats[channel]['correct_predictions'] += 1
    ...
    with open(csv_filepath, 'w', newline='') as csvfile:
        fieldnames = ['Channel', 'Total_Images', 'Total_Annotations', 'Total_Predicted_Boxes', 'Correct_Predictions', 'Missing_Predictions']
        ...
        for channel, stats in channel_stats.items():
            writer.writerow({
                ...
                'Missing_Predictions': stats['total_annotations']-stats['total_predicted_boxes']
            })
