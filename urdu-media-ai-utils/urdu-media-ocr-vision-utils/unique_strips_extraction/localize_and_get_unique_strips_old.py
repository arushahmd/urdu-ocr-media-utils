...
def differentiate_news_strips_orb(news_strips):
    ...
    # Select one instance from each cluster
    final_uniques = [cluster[0] for cluster in similar_clusters]

    # Filter out blank strips based on aspect ratio
    not_blank_strips = [strip for strip in final_uniques if (strip_width := strip[-3].shape[0]) / strip[-3].shape[1] < 0.25]
