...
"""
-------------- _load_images----------------------
This Method Load Images from Handle training 

Input parameter: 
filenames = image file name
image_size=image size (define in config file)
flip_image= Status to flip image or not default set = true
backets= contains the size of the buckets to incorperate the different image sizes to different buckets default buckets=1

Return:
images= contains all the images   
seq_len=contain sequence length of images
bucket_indices =contain bucket length of images
"""
...
"""
-------------- _size_alright----------------------
This Method give the status about the image_size

Input parameter: 
directory = Contains the image path
img_width=image width size default=none
image_range= Contains the image_range default=none

Return:
Status =about the image shape satisfied the conditions / or not.
"""
...
# Load the load_gt data by returing images ground truth, filenames 
...
# Load the _bucket_gt data by returing images_Label, images_Label_seq_len 
...
# load the entire data and returning the images, images_seq_len, images_Label, images_Label_seq_len 
...
# By calling the load_gt by passing the (dictionary ,Image_directory,Image_width,Image_range,size) 
# Returning the output as groundtruths and their filenames
    groundtruths, filenames = load_gt(gt_directory,
                                      image_directory=image_directory,
                                      image_width=image_size[0],
                                      image_range=image_width_range,
                                      size=size)
# By calling the _load_images by passing filenames,image size(none,64) , flip_image flag, buckets size(5)
# Returning the images, images_seq_len and bucket_indices.
    images, images_seq_len, bucket_indices = _load_images(filenames,
                                                          image_size,
                                                          flip_image=flip_image,
                                                          buckets=buckets)

# By calling the _bucket_gt by passing bucket_indices, buckets (5) and groundtruths of file.
# Returning the images_Label, images_Label_seq_len .
...
#It create the bins and bucket_indices w.r.t the bins
...
def convert_to_urdu(output, data_folder):
    ...
    # print("Characters :", urdu_output)
    return urdu_output, join_char

#Load the Look_up table in the labels folder
