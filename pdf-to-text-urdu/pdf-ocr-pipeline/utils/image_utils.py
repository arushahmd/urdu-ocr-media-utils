...
class ImagePreprocessing():
    ...
    def __call__(self, image_path, save_path=None):
        # Load the paremeters
        params = self.params
        # Read the image form image_path
        ...
        # exit()
        # print(image_path)
        # Split the image name and type
        # name, image_type = os.path.split(image_path)[-1].split('.')
        # if extract_red_component is true then extracted the red_component
        if params.extract_red_component:
            images = [_extract_red_component(images[0])]
        # If horizontal_segmentation is true then horizontally segmantized the image
        if params.horizontal_segmentation:
            ...
            images = all_images
        # If vertical_segmentation is true then horizontally segmantized the image
        if params.vertical_segmentation:
            all_images = []
            for image in images:
                new_images = _vertical_segmentation(image, VERTICAL_MIN_AREA, merge_contours=True,
                                                    correct_skew=params.correct_skew)
                all_images.extend(new_images)
            images = all_images
        # If remove_white_columns is true then Remove the white columns
        if params.remove_white_columns:
            images = _remove_white_columns(images)
            # GETTING IMAGE CORRECTLY

        # If remove_white_rows is true then Remove the white rows
        if params.remove_white_rows:
            images = _remove_white_rows(images)

        if params.write_images:
            if save_path is None:
                raise ValueError("Need to provide save_path to write images")
            # Save the cahnge_Images into path by same file_name and file_type.
...
def _extract_red_component(image):
    ...
    ## mask for red pixels and the blue and green pixel condition
    ## We use only the red_pixel because the data set consist of red pen write (black pen mixes with the backgroung lines)
...
def _remove_white_columns(image):
    ...
    image = image[:, ~mask]  # Remove White_Columns
...
def _remove_white_rows(image):
    ...
    image = image[~mask, :]  # Remove White_Rows
...
# for removing the extra spaces in a line
...
# binarized image means turn the Gray image into the numbers from 0(white) to 255(black) w.r.t the image pixels
# intensity
def _binarize(image, blur=True):
    # Defining window size and value of k which will be different for each channel
    window_size = 94
    k = 0.3
    # Geo News For now
    
    from  .binarization import get_gray_image, convert_binary
    # cv2.imshow("Original", image)

    
    # img_path = os.path.join(fold_path, j)
...
def _trim_image(image):
    ...
    # Blur the image by using Gaussian Blur
    blur = cv2.GaussianBlur(gray, (5, 5), 0)
    thresh = cv2.bitwise_not(cv2.threshold(blur, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)[1].astype(np.uint8))
    # Find the non zero cordinated of the image
    coords = cv2.findNonZero(thresh)
    x, y, w, h = cv2.boundingRect(coords)
    # Remove the extra white pixel to crop the image
...
# resize The images by calling the _resize_image method
def _resize_images(image, image_size, flip_image=True, buckets=1):
    features = []
    seq_len = []

    # If Image size for all images is specified then bucket size should be 1 to convert all images to standardized size
    ...
    # for image in images:
    # if VERBOSE:
    # HERE THE IMAGE IS BEING SHOWED cv2.imshow()
    # cv2.imshow("original image", image)
    # cv2.waitKey(0)

    # Call the _resize_image method by passing the binarizing the the image
    ...
    features.append(f)
    seq_len.append(s)
    # if buckets=1 then all the images in one buckets then bucket_indices variable is none
    if buckets == 1:
        features = [features]
        seq_len = [np.array(seq_len)]
        bucket_indices = None
    # if buckets!=1 then all the images in to corresponding buckets
    else:
    ...
    # Return the restze image features
...
def _resize_image(image, image_size, flip_image=True):
    (wt, ht) = image_size
    (h, w) = image.shape
    # If Image  wodth is not defiend in our case wt is always none because the in config file none,64
    if wt is None:
        f = ht / h
        new_size = (int(w * f), ht)
    else:
    ...
    # Copy the resize_image into target with flip/not_flip
    # _normalized the image meaning take image and map into the 2d-corrdinates of 0,0 center.
    if wt is not None:
...
# Map the image 0,0 center by their STD and mean
...
# append Zeros horrizontally to convert all images into one size
...
# Copy the image into target with flip/not_flip
...
# Call by the data_utiles.py To load the train dataset images
# It Return the images in file_names, seq_lenth and bucket_indices
def handle_training(filenames, image_size, flip_image=True, buckets=1, params=TRAINING):
    preprocessor = ImagePreprocessing(params)

    images = []
    # Append all the preprocessed images to image array...
    for filename in filenames:
        images.append(preprocessor(filename, save_path=None)[0])
    # _resize_images resize the image and return the image, Iamge_seq_len and bucket_indices
...
def handle_inferring(image_strip, image_size, flip_image=True, buckets=1, params=INFERRING):
    """
    ...
    # if type(image_strip) == np.ndarray:
    # filenames = paths
    ...
    #
    #         else:
    #             raise ValueError("%d element in 'paths' is not a readed image nd array to a file or a directory" % i)
    #
    # else:
    ...
    # for f in filenames:
    # IMAGE IS PASSING CORRECTLY
    image_strip = preprocessor(image_strip, save_path=None)
    # GETTING IMAGE CORRECTLY
    images, seq_len, _ = _resize_images(image_strip, image_size, flip_image=flip_image, buckets=buckets)
