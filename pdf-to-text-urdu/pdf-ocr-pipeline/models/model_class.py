...
from ..utils.helpers import from_sparse
from ..utils.image_utils import *
from ..utils.accuracy_metrics import *
from .lm import ctc_prefix_beam_search_decoder

plt.switch_backend('agg')
logging.basicConfig(level=logging.INFO)
tf.logging.set_verbosity(tf.logging.ERROR)
# Initialized the Split Ratio to create validate and train data from labeled Input Data
SPLIT_RATIO = 0.80
TIMEOUT = 3


class Model():
    # print("model_class.py -> Model.__call__() ")
    # Initialize the instancs of the class
    def __init__(self, config, eval_path=None, infer=True):
        # print("class Model -> __init__")
        tf.reset_default_graph()  # In case some tensorflow graph is already loaded
        self.config = config
        # making sure either training or inference one at a time
        ...
        self.INFER = infer
        # Save directory that given in cofig file
        # If not having dir. than create the dir. into the given path
        # And During the training phase only load the training data
        # if self.TRAIN:
        ...
        # During the Evalating phase only load the evaluate data
        # if self.EVALUATE:
        #     self._load_evaluation_data(eval_path)
        # Initialize graph and constant
        self._handle_constants()
        # print("    -- calling _initialize_graph")
        self._initialize_graph()
        # If Inference is set true to initialize inference model
        # Inference Means Prediction Model
        if self.INFER:
            self._initialize_model(True, verbose=False)

    # Instance is call of the Model class and infenece is true then return the inference Out_put otherwise none
    def __call__(self, path=None):
        # If the Model is in Training Phase then the method Loaded the Model result from _fit Method and Plot the Result
        if self.TRAIN:
            logging.info("-" * 50 + "\nStarting Training\n" + "-" * 50)
        ...
        # elseIf The model in evaluatuion phase then Call the _evaluate Method
        elif self.EVALUATE:
            self._evaluate()
        # elseIf The model in Inference phase(Model Prediction Phase) then Call the _infer Method by loading the
        # inferenc dataset from the path
        elif self.INFER:
            # print("    + model_class.py -> _load_inferring_data() ")
            self._load_inferring_data(path)

            # print("Data is loaded and pre-processed sending now for inference ...")
            out = self._infer()  # Got here the transcripted text like [28, 49, 14, 3]
            # print("---------- ", out)
            urdu_out = [convert_to_urdu(o, self.config.data_folder)[1] for o in out]

        return (out, urdu_out) if self.INFER else None

    # This Method Laod the training data
    def _load_training_data(self):
        # Load the Config instance
        config = self.config
        # Load the Training Data from the _load_data_from_folder method and return the input, input_seq_lenght , label and label_seq_length
        X, X_seq_len, y, y_seq_len = self._load_data_from_folder(config.data_folder, size=config.num_train)
        # determine the vocabuary size fron the labeled output
        self.vocab_size = int(max([max(yij) for yi in y for yij in yi]) + 1)
        self.y_max_len = np.max([np.max(s) for s in y_seq_len])
        # SAve the Model and directory in save_dir
        save_dir = os.path.join(config.save_dir, config.model)
        # Our dataset consist of the input, input_seq_lenght , label and label_seq_length
        dataset = (X, X_seq_len, y, y_seq_len)
        # If restore_previous_model is true then load the indices from the previous model otherwise none
        if self.config.restore_previous_model:
            indices = pickle.load(open(os.path.join(config.save_dir, config.model, "split_indices.pkl"), "rb"))[
                "indices"]
        else:
            indices = None
        # It prepare Dataset and returning the validation, train dataset w.r.t split_ratio and indices..
        self.train_dataset, self.val_dataset, indices = prepare_dataset(dataset,
                                                                        r=SPLIT_RATIO,
                                                                        shuffle=True,
                                                                        all_indices=indices)
        # Stored newely obtained indices in file
    ...
    def _load_evaluation_data(self, eval_path):
        ...
        dataset = (X, X_seq_len, y, y_seq_len)
        # It prepare test Dataset and returning the evaluation dataset
    ...
    def _load_inferring_data(self, images):
        config = self.config
        # IMAGE IS PASSING CORRECTLY
        self.X_infer, self.X_infer_seq_len = handle_inferring(images,
                                                              config.image_size,
                                                              flip_image=config.flip_image,
                                                              buckets=1)
        # cv2.imshow("Processed Image", self.X_infer[0])
        # cv2.waitKey(0)
        # cv2.destroyAllWindows()
        # result = cv2.normalize(self.X_infer[0], dst=None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_8U)
        # cv2.imwrite('Flipped.jpg', result)

    # Load data from the data_folder by passing the data_folder path and image configuration
    # Returing the images, images_seq_len, label, label_sq_length
    ...
    def _handle_constants(self):
        config = self.config
        # Save the current model in constrants.pkl
        save_dir = os.path.join(config.save_dir, config.model, "constants.pkl")
        # If in training phase and using teacher forcing
        if self.TRAIN and not config.restore_previous_model:
            if config.use_teacher_forcing:
                # Start and End of sentence mention and also mention padding because we embadedd d3
                self.extra_codes = {'<SOS>': self.vocab_size, '<EOS>': self.vocab_size + 1,
                                    '<PAD>': self.vocab_size + 2}
                self.vocab_size += 3
                self.y_max_len += 1

            with open(save_dir, 'wb') as f:
                # Save the vocabluray size in directory
            ...
            # If not in training phase then loab the sos, eos and pad w.r.t ??
            if config.use_teacher_forcing:
                self.extra_codes = {'<SOS>': self.vocab_size - 3, '<EOS>': self.vocab_size - 2,
                                    '<PAD>': self.vocab_size - 1}
        # Create a log of vocabulary_size and Maximum Length of Output Sequence
        if self.TRAIN:
    ...
    def _placeholders(self):
        config = self.config
        # Create the placeholders that uses in model implementation
        self.X_placeholder = tf.placeholder(tf.float32, [None, config.image_size[0], config.image_size[1]],
                                            name="X_placeholder")
        self.X_seq_len_placeholder = tf.placeholder(tf.int32, [None], name="X_seq_len_placeholder")
        # If Teacher forcing apply then not needed to initialize place holder for y_size, y_length and y_values otherwise needed
        if config.use_teacher_forcing:
        ...
        # y_placeholder is made of SparseTensor is consist of more than one placeholder
        else:
        ...
        # is_training placeholder is needed for discribe wether the training is needed or not
        ...
        # Also initilaize the placeholder for droupout in model
    ...
    def _create_feed_dict(self, X, X_seq_len, y=None, y_seq_len=None, dropout=None, is_training=True):
        ...
        feed_dict = {self.X_placeholder: X, self.X_seq_len_placeholder: X_seq_len,
    ...
    def _get_output_indices(self, preds, labels=None, num_examples=None, test_flag=False):
        ...
        if config.use_sparse_labels:
            ...
            # print("        ++ _get_output_indeces() Preds :", preds)
    ...
    def _train_on_batch(self, dataset, dropout, sess, find_accuracy=False):
        ...
        if find_accuracy:
            ...
            _, grad_norm, loss, summary = self.sess.run([self.train_op, self.grad_norm, self.loss, self.merged_summary],
                                                        feed_dict)
    ...
    def _train(self, dataset, sess, histories):
        ...
        for i, mini in enumerate(minibatches(dataset, config.batch_size, shuffle=True)):
            if i % config.print_every == 0:
                tic = time.time()
                grad_norm, loss, summary, accuracy = self._train_on_batch(mini, config.dropout, sess,
                                                                          find_accuracy=True)
    ...
    def _validate(self, dataset, sess):
        ...
        for i, mini in enumerate(minibatches(dataset, config.batch_size, shuffle=False)):
            ...
            loss, decoded_train, decoded_infer = sess.run([self.loss, self.decoded_train, self.decoded_infer],
                                                          feed_dict_val)
    ...
    def _fit(self):
        ...
        for epoch in range(config.num_epochs):
            ...
            if (config.save_best and (histories['best_val_loss'] == None or val_loss <= histories[
                'best_val_loss'])) or not config.save_best:
        ...
        self._log_validation_results(histories['best_val_loss'], histories['best_val_train_accuracy'],
                                     histories['best_val_infer_accuracy'])
    ...
    def _infer(self):
        # print("    + model_class.py -> _infer() ")
        dropout = 0.0 if self.config.dropout is not None else None

        # cv2.imshow("Image", self.X_infer[0])
        # cv2.waitKey(0)
        # cv2.destroyAllWindows()
        # print(self.X_infer_seq_len)

        # X_infer is inputted image
        feed_dict = self._create_feed_dict(self.X_infer, self.X_infer_seq_len, y=None, y_seq_len=None, dropout=dropout,
                                           is_training=False)

        # This will not run
        if self.config.save_alignments:
            decoded, alignment = self.sess.run([self.decoded_infer, self.infer_alignment], feed_dict)
            _create_attention_video(self.X_infer, alignment, self.config.save_dir, decoded, self.config.data_folder,
                                    self.extra_codes)

        else:
            # print(self.decoded_infer)
            # print("DEBUG...........")
